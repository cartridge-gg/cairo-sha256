...
from src.packed_sha256 import BLOCK_SIZE, compute_message_schedule, sha2_compress, get_round_constants
...
func sha256{range_check_ptr, sha256_ptr: felt*, sha256_ptr_end: felt*}(data: felt*, n_bytes: felt) -> (
    output : felt*
):
    alloc_locals

    let (local q, r) = unsigned_div_rem(n_bytes, SHA256_INPUT_CHUNK_SIZE_BYTES)
    let sha256_ptr_end = sha256_ptr + (q + 1 * SHA256_INPUT_CHUNK_SIZE_FELTS)

    # Set the initial state to IV.
    assert sha256_ptr_end[0] = 0x6A09E667
    assert sha256_ptr_end[1] = 0xBB67AE85
    assert sha256_ptr_end[2] = 0x3C6EF372
    assert sha256_ptr_end[3] = 0xA54FF53A
    assert sha256_ptr_end[4] = 0x510E527F
    assert sha256_ptr_end[5] = 0x9B05688C
    assert sha256_ptr_end[6] = 0x1F83D9AB
    assert sha256_ptr_end[7] = 0x5BE0CD19
    let sha256_ptr_end = sha256_ptr_end + SHA256_STATE_SIZE_FELTS
    let output = sha256_ptr_end
...
# before the message.
func sha256_inner{range_check_ptr, sha256_ptr: felt*, sha256_ptr_end: felt*}(
    ...
    let (n) = _sha256_input(data, n_bytes, SHA256_INPUT_CHUNK_SIZE_FELTS)
...
func _sha256_input{range_check_ptr, sha256_ptr: felt*}(
    ...
    if n_bytes == 0:
        let (end_block) = is_le(n_words, 14)
        if end_block == 1:
...
func sha256_last_block{range_check_ptr, sha256_ptr: felt*, sha256_ptr_end: felt*}(
    data: felt*, n_bytes: felt, total_bytes: felt
):
    alloc_locals
